...
//try to find the first node which not match the user defined Greater() condition
func (s *SkipList) search_GreaterEqual(o SkipListObj) (*Node, error) {
	p := s.head

	for i := s.maxLevel - 1; i >= 0; i-- {
		for {
			if p.forward[i] != nil && p.forward[i].O.Greater(o) {
				p = p.forward[i]
			} else {
				break
			}
...
func (s *SkipList) Search(o SkipListObj) (SkipListObj, error) {
	...
	res, err := s.search_GreaterEqual(o)
	if err == nil {
		if res.O.Greater(o) {
			return res.O, nil
		} else {
...
func (s *SkipList) SearchRange(minObj, maxObj SkipListObj) ([]SkipListObj, error) {
	...
	p, err := s.search_GreaterEqual(minObj)
	if err != nil {
		return res, err
	}

	for {
		if p != nil && p.O.Greater(maxObj) {
			res = append(res, p.O)
			p = p.forward[0]
		} else {
			break
...
func (s *SkipList) Insert(obj SkipListObj) (bool, error) {
	...
	for i := s.maxLevel - 1; i >= 0; i-- {
		for {
			if p.forward[i] != nil && p.forward[i].O.Greater(obj) {
				p = p.forward[i]
			} else {
				break
			}
		}
		//find the last Node which match user defined Greater() condition in i level
...
func (s *SkipList) RemoveNode(obj SkipListObj) (bool, error) {
	...
	if p == nil || p.O.Greater(obj) || obj.Greater(p.O) {
